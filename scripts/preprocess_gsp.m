...
root_path = '/gs/project/gsf-624-aa/simons_vip/GSP_all/';

path_raw = [root_path 'gsp_raw/'];
...
for num_s = 1:length(list_subject)
    ...
    files_in.(id).fmri.session1.rest = [tmp_path_subj dir([tmp_path_subj '*BOLD1.nii.gz'])(1).name];  
    rest2_flag_exist = dir([tmp_path_subj '*BOLD2.nii.gz']);
    if isempty (rest2_flag_exist) 
        warning('Subject %s is missing either the rest2',subject)
    else
        files_in.(id).fmri.session2.rest = [tmp_path_subj dir([tmp_path_subj '*BOLD2.nii.gz'])(1).name];
    end
  
    files_in.(id).anat = [tmp_path_subj dir([tmp_path_subj '*ANAT1.nii.gz'])(1).name];
end

%% Pipeline options  %%
%% General
opt.folder_out   = [root_path 'prep_out/'];     
...
% psom options
opt.psom.qsub_options = '-A gsf-624-aa -q sw -l walltime=10:00:00';
opt.psom.max_queued = 800;
